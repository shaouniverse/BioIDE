...
function addTaxkey() {
    ...
    //检索表样式切换
    $("input:radio[name='taxkeyType_" + (countTaxkey + 1) + "']").change(function () {
        var taxkeyType = $("input[name=taxkeyType_" + (countTaxkey + 1) + "]:checked").val();
        switch (taxkeyType) {
            case "option1":
                layer.msg('切换至【双项式】检索表',
                    ...
                    function () {
                        ...
                        loadKeytable((countTaxkey + 1), taxkeyData);
                    });
                break;
            case "option2":
...
function buildKeytable(taxkeyNum) {
    ...
    $(function () {
        $table.bootstrapTable({data: taxkeyDefaultData});
    });
}
